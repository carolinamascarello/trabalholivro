...
while (codigoConsulta !== -1) {
    codigoConsulta = parseInt(prompt("Digite o código da obra (-1 para sair, -2 para adicionar uma obra):"));
    if (codigoConsulta === -1) {
        break;
    } else if (codigoConsulta === -2) {
        adicionarObra();
    } else {
        const areaConsulta = prompt("Digite a área da obra:");
        const obraConsultada = consultarObraPorCodigoArea(codigoConsulta, areaConsulta);
        if (obraConsultada) {
